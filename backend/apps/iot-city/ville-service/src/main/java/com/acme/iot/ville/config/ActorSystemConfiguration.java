...
import com.acme.iot.ville.messaging.SensorReadingHandler;
...
public class ActorSystemConfiguration {
    ...
    private SensorReadingHandler sensorReadingHandler;
    ...
    public InterPodMessaging interPodMessaging() {
        try {
            ...
            logger.info("InterPodMessaging initialized for ville-service");
    ...
    public RemoteMessageTransport kafkaMessageTransport(InterPodMessaging messaging) {
        ...
        logger.info("KafkaMessageTransport created for ville-service");
        return transport;
    }
    ...
    public ActorSystem actorSystem(HttpVilleActorFactory actorFactory, 
        ...
        logger.info("KafkaActorMessageHandler started for ville-service");
        
        // Initialize sensor reading handler to receive sensor data from capteurs
        this.sensorReadingHandler = new SensorReadingHandler(system, messaging);
        sensorReadingHandler.start();
        logger.info("SensorReadingHandler started for ville-service");
    ...
    public void shutdown() {
        logger.info("Shutting down ville-service messaging...");
        
        if (sensorReadingHandler != null) {
            sensorReadingHandler.stop();
        }
